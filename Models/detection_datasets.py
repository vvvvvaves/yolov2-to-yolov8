...
class VOCDatasetV2(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # initialize tensors
        with record_function("gt_out init"):
        ...
        for box in bboxes:
            with record_function("Box processing"):
                ...
                for scale_idx, scale in enumerate(self.scales):
                    ...
                    bndbox = torch.tensor([obj_xc, obj_yc, obj_w, obj_h], dtype=self.dtype, device=self.device)
                    
                    # print((cell_x, cell_y), bndbox, self.classes[class_label])
                    
                    IoUs = torch.empty(len(self.anchors), dtype=self.dtype, device=self.device)
                    for i, anchor in enumerate(self.anchors):
                        cell_aw = anchor[0] * scale
                        cell_ah = anchor[1] * scale
                        _anchor = torch.tensor([obj_xc, obj_yc, cell_aw, cell_ah], dtype=self.dtype, device=self.device)
